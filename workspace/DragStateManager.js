...
export class DragStateManager {
    ...
    start(x, y) {
        ...
        let intersects = this.raycaster.intersectObjects(this.scene.children, true); // true = recursive
        console.log(`DragStateManager: Found ${intersects.length} intersections`);
        
        for (let i = 0; i < intersects.length; i++) {
            let obj = intersects[i].object;
            console.log(`Checking object:`, obj.name || obj.type, `bodyID:`, obj.bodyID);
            
            if (obj.bodyID && obj.bodyID > 0) {
                console.log(`Found draggable body: ${obj.bodyID}`);
        ...
        if (!this.physicsObject) {
            console.log('No draggable object found');
        }
    }
    move(x, y) {
        if (this.active) {
